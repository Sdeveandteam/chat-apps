...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      ...
      [username.trim().toLowerCase(), hash, publicKey]
    );
...
io.on('connection', (socket) => {
  ...
  socket.on('send_message', async ({ toUserId, ciphertext, iv }) => {
    try {
      const result = await pool.query(
        'INSERT INTO messages (sender_id, receiver_id, ciphertext, iv) VALUES ($1,$2,$3,$4) RETURNING id, created_at',
        [userId, toUserId, ciphertext, iv]
      );
      const payload = {
        id: result.rows[0].id,
        fromUserId: userId,
      ...
      if (targetSocket) io.to(targetSocket).emit('receive_message', payload);
